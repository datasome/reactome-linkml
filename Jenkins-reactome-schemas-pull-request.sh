...
docker run reactome-schemas-pull-request:latest sh -c './generate.py yaml schema.web.diff.yaml && gen-json-schema schema.web.yaml 1> /dev/null 2> err.log; if [ "$?" -ne "0" ]; then echo "schema.web.yaml syntax is not valid: "`cat err.log`; exit 1; fi'
